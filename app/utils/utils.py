...
def databricks_half_mask_column(column_name: str) -> str:
    """
    Generates a SQL snippet for Databricks to half-mask the given column's text value.

    The first half of the string (minimum 1 and maximum 4 characters) is preserved, 
    while the remainder is replaced with asterisks ('*'), based on the column's length.

    Args:
        column_name (str): The name of the column to half-mask.

    Returns:
        str: A SQL snippet that, when used in a query, will half-mask the values of the specified column.
    # sql snippet to query databricks by half-mask text
    """
    snippet = f"""
        concat(
        substr({column_name}, 1, least(greatest(CAST(floor(length({column_name})/2) AS INT), 1), 4))
        , repeat('*', length({column_name}) - least(greatest(CAST(floor(length({column_name})/2) AS INT), 1),4))
        )
        """
    return snippet
...
def convert_portfolio_to_df(portfolio: Portfolio) -> pd.DataFrame:
    ...
    if "value" not in df.columns:
        if "marketValue" in df.columns:
            ...
            logger.error(
                f"No value column found! Available columns: {list(df.columns)}"
            )
            raise ValueError(
                "Portfolio must have a value/marketValue/mkt_val_thb column"
            )
    ...
    if missing_mask.any():
        logger.error(
            f"Found {missing_mask.sum()} positions with missing src_sharecodes"
        )
    ...
    if "src_symbol" not in df.columns:
        df["src_symbol"] = (
            df["symbol"] if "symbol" in df.columns else df["src_sharecodes"]
        )

    if "symbol" not in df.columns:
        df["symbol"] = (
            df["src_symbol"] if "src_symbol" in df.columns else df["src_sharecodes"]
        )
