...
class PortpropMatrices:
    ...
    def load_portprop_ref_tables(self):
        df_port_fs = read_sql(
            "SELECT * FROM user.edg.portprop_factsheet WHERE symbol IS NOT NULL"
        )
        ...
        df_ge_mapping = read_sql("SELECT * FROM user.edg.portprop_ge_mapping")

        df_port_ret = read_sql("SELECT * FROM user.edg.portprop_ret_eow")

        df_model = read_sql("SELECT * FROM user.edg.advisory_health_score")
    ...
    def cal_ret_cov(self):
        df_ret_pivot = self.df_port_ret.pivot_table(
            index="RETURN_DATE", columns="BM_NAME", values="RETURN", aggfunc="sum"
        )
    ...
    def cal_df_out_join_bm(self, ports):
        ...
        df_out_join["ASSET_ALLO_FALLBACK_JOIN_KEY"] = np.where(
            (df_out_join["ASSET_ALLO_JOIN_KEY"].notna())
            & (df_out_join["SYMBOL_FS"].isna()),
            "FALLBACK",
            np.nan,
        )
        ...
        df_out_join["ASSET_CLASS"] = np.where(
            ...
            np.where(
                ~fallback, df_out_join["ASSET_CLASS"], df_out_join["ASSET_CLASS_FB"]
            ),
        )
        df_out_join["BM_NAME"] = np.where(
            ~is_alloc,
            df_out_join["BM_NAME_BM"],
            np.where(~fallback, df_out_join["BM_NAME"], df_out_join["BM_NAME_FB"]),
        )
        ...
        df_out_join = df_out_join[
            list(ports.df_out.columns) + ["ASSET_CLASS", "BM_NAME"]
        ]
    ...
    def get_asset_class_allocation(self, ports):
        ...
        df = self.df_out_join
        df_astc_alloc = (
            df.groupby(["PORT_ID", "ASSET_CLASS"])["WEIGHT"]
            .sum()
        ...
        df_comp_astc = (
            df.groupby(["PORT_ID"] + ports.prod_comp_keys + ["ASSET_CLASS"])["WEIGHT"]
            .sum()
            .unstack(fill_value=0)
            .reindex(columns=all_classes, fill_value=0)
            .rename(columns=asset_class_map)
    ...
    def get_model_asset_class_allocation(self, ports):
        model_allocation = ports.df_style.merge(
            self.df_model,
        ...
        model_allocation[self.asset_class_list] = (
            model_allocation[self.asset_class_list] / 100
        )
    ...
    def get_model_asset_class_deviation(self, ports):
        ...
        model_allocation = ports.df_style.merge(
            df_astc_alloc, on="PORT_ID", how="left"
        ).merge(
            self.df_model,
            left_on="PORTPOP_STYLES",
            right_on="MODEL_NAME",
        ...
        for x in self.asset_class_list:
            model_allocation[x + "_DIF"] = (
                model_allocation[x] - model_allocation[x + "_MODEL"] / 100
            )
            diff_col.append(x + "_DIF")

        return model_allocation[
            ["PORT_ID", "PORT_INVESTMENT_STYLE", "PORTPOP_STYLES"] + diff_col
        ]

    def get_geography_equity_allocation(self, ports):
        df_out_join_ge = self.df_out_join[
            self.df_out_join["ASSET_CLASS"] == "Global Equity"
        ]
        df_out_join_ge_sum = df_out_join_ge.groupby(["PORT_ID"])["WEIGHT"].sum()
        df_out_join_ge = df_out_join_ge.merge(
            df_out_join_ge_sum.rename("WEIGHT_SUM"),
            left_on="PORT_ID",
            right_index=True,
            how="left",
        )
        ...
        for col in self.equity_geography_list:
            df_out_join_ge[f"WEIGHTED_{col}"] = (
                df_out_join_ge["WEIGHT"] * df_out_join_ge[col]
            ) / df_out_join_ge["WEIGHT_SUM"]
        ...
        df_ge_alloc = (
            df_out_join_ge.groupby("PORT_ID")[alloc_cols]
            .sum()
            .rename(
                columns={
                    "WEIGHTED_EM": "GE_EM",
        ...
        df_comp_ge = (
            df_out_join_ge.groupby(["PORT_ID"] + ports.prod_comp_keys)[alloc_cols]
            .sum()
            .rename(
                columns={
                    "WEIGHTED_EM": "GE_EM",
    ...
    def get_expected_return(self, ports):
        self.df_out_join["WEIGHTED_RETURN"] = (
            self.df_out_join["WEIGHT"] * self.df_out_join["EXPECTED_RETURN"]
        )
        df_exp_ret = (
            self.df_out_join.groupby("PORT_ID", as_index=False)["WEIGHTED_RETURN"]
            .sum()
            .rename(columns={"WEIGHTED_RETURN": "EXPECTED_RETURN"})
        )
        df_comp_exp_ret = (
            self.df_out_join.groupby(
                ["PORT_ID"] + ports.prod_comp_keys, as_index=False
            )["WEIGHTED_RETURN"]
            .sum()
            .rename(columns={"WEIGHTED_RETURN": "EXPECTED_RETURN"})
        )
    ...
    def get_volatility(self, ports):
        ...
        df_weights = (
            self.df_out_join.groupby(["PORT_ID", "BM_NAME"])["WEIGHT"]
            .sum()
            .unstack(fill_value=0)
        ...
        stdev_ret_matrix = np.sqrt(
            (weight_matrix @ cov_matrix * weight_matrix).sum(axis=1)
        )
        ...
        marginal_risk = (
            np.sqrt(52)
            * weight_matrix
            * (weight_matrix @ cov_matrix)
            / np.where(stdev_ret_matrix[:, None] == 0, 1, stdev_ret_matrix[:, None])
        )

        df_product_weights = (
            self.df_out_join.groupby(["PORT_ID", "BM_NAME"] + ports.prod_comp_keys)[
                "WEIGHT"
            ]
            .sum()
            .reset_index()
        ...
        risk_contrib_df = (
            pd.DataFrame(marginal_risk, index=ports.port_ids, columns=bm_names)
            .stack()
        ...
        df = df_product_weights.merge(
            risk_contrib_df, on=["PORT_ID", "BM_NAME"], how="left"
        )

        total_bm_weight = df.groupby(["PORT_ID", "BM_NAME"])["WEIGHT"].transform("sum")
        df["VOL_CONTRIBUTION"] = (
            df["WEIGHT"] * df["RISK_CONTRIBUTION"] / total_bm_weight
        )
    ...
    def get_portprop_matrix(self, ports):
        ...
        df_port_matrix = (
            ports.df_style.merge(df_vol, on="PORT_ID", how="left")
