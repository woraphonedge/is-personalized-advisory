...
class Rebalancer:
    ...
    def __init__(
        ...
        self.product_whitelist = (
            [p.lower() for p in product_whitelist] if product_whitelist else []
        )
        self.product_blacklist = (
            [p.lower() for p in product_blacklist] if product_blacklist else []
        )
        self.discretionary_acceptance = (
            0.5 if discretionary_acceptance is None else float(discretionary_acceptance)
        )
    ...
    def set_ref_tables(self, ref_dict: dict):
        ...
        self.es_sell_list = (
            None if "es_sell_list" not in ref_dict else ref_dict["es_sell_list"]
        )
        self.prod_reco_rank_raw = (
            None
            if "product_recommendation_rank_raw" not in ref_dict
            else ref_dict["product_recommendation_rank_raw"]
        )
        self.discretionary_allo_weight = (
            None
            if "mandate_allocation" not in ref_dict
            else ref_dict["mandate_allocation"]
        )
        ...
        for key, value in required_elements.items():
            if value is None:
                warnings.warn(
                    f"'{key}' not provided. Expect errors if used.",
                    UserWarning,
                    stacklevel=2,
                )
    ...
    def get_product_recommendation_rank(self, ports) -> pd.DataFrame:
        ...
        if not hasattr(self, "prod_reco_rank_raw") or self.prod_reco_rank_raw is None:
            ...
            logger.warning(
                "prod_reco_rank_raw is None; continuing with empty recommendations"
            )
            self.prod_reco_rank_raw = pd.DataFrame(
                columns=["src_sharecodes", "desk", "currency"]
            )  # minimal expected keys
        ## BUG: join with ["src_sharecodes", "desk", "currency"] is not uniuqe
        prod_reco_rank = self.prod_reco_rank_raw.join(
            ports.product_mapping, on="sec_id", how="left", rsuffix="_mapping"
        )
    ...
    # Cash Proxy funding
    # ---------------------------
    def build_cash_proxy_funding(
        self, ports, action: pd.DataFrame, per_row: bool = True
    ) -> pd.DataFrame:
        """
        ...
        if ports.product_mapping is None:
            raise RuntimeError(
                "Product mapping must be loaded before mapping cash proxy."
            )
        ...
        funding = action[["port_id", "currency", "amount"]].merge(
            cash_map, on="currency", how="left"
        )
    ...
    def get_cash_proxy(self, ports) -> pd.DataFrame:
        ...
        if cash.empty:
            return cash
        cash_ccy = (
            cash.groupby(["port_id", "currency"], as_index=False).agg(
                weight=("weight", "sum"), value=("value", "sum")
            )
    ...
    def update_portfolio(self, ports: "Portfolios", actions: pd.DataFrame) -> None:
        ...
        current_positions = self.new_ports.df_out[
            ["port_id", "value"] + self.new_ports.prod_comp_keys
        ].copy()

        # Combine current and actions positions
        new_positions = pd.concat(
            [current_positions, action_positions], ignore_index=True, sort=False
        )

        # Aggregate positions by product keys
        new_positions = new_positions.groupby(
            ["port_id"] + self.new_ports.prod_comp_keys,
            dropna=False,
            as_index=False,
        )["value"].sum()

        df_out = new_positions.join(
            self.new_ports.product_mapping, on="sec_id", how="left", rsuffix="_y"
        )
        df_out.drop(columns="sec_id_y", inplace=True)

        # Filter out zero positions
        df_out = df_out[df_out["value"] != 0]

        # Update new ports
        self.new_ports.set_portfolio(
            df_out,
            self.new_ports.df_style,
            self.new_ports.port_ids,
            self.new_ports.port_id_mapping,
        )
    ...
    def select_product_discretionary_by_need(
        ...
        need = np.array(
            [
                max(float(diffs[f"{c}_model"].iloc[0] - diffs[c].iloc[0]), 0.0)
                for c in aa_cols
            ],
            dtype=float,
        )
        ...
        cands = self.discretionary_allo_weight[
            self.discretionary_allo_weight["currency"] == currency
        ].copy()
        if cands.empty:
            return None
        ...
        min_dist = dists.min()
        winners = cands.iloc[
            np.where(np.isclose(dists, min_dist, atol=self.eps_tiny))[0]
        ]
    ...
    def select_product_discretionary_by_style(
        ...
        if self.client_investment_style is None and ports.df_style is not None:
            self.client_investment_style = ports.df_style.iloc[0][
                "port_investment_style"
            ]
        elif self.client_investment_style is None and (
            ports.df_style is None or ports.df_style.empty
        ):
            return None
        ...
        if self.client_investment_style in style_discretionary_mapping:
            allowed_sharecodes = set(
                style_discretionary_mapping[self.client_investment_style]
            )
    ...
    def get_current_discretionary_weight(self, ports) -> float:
        discretionary_syms = set(
            self.discretionary_allo_weight["src_symbol"].astype(str).unique()
        )
        if "symbol" in ports.df_out.columns:
            return float(
                ports.df_out.loc[
                    ports.df_out["symbol"].astype(str).isin(discretionary_syms),
                    "weight",
                ].sum()
                or 0.0
            )
    ...
    @staticmethod
    def check_not_monitored_from_comp(comp: pd.DataFrame) -> pd.DataFrame:
        mask = (
    ...
    @staticmethod
    def check_issuer_risk_from_comp(comp: pd.DataFrame) -> pd.DataFrame:
        ...
        sub = sub.merge(
            sum_weight,
            on="issure_risk_group",
            how="left",
            suffixes=("", "_issure_risk_group"),
        )
        sub["expected_weight"] = (0.19 / sub["weight_issure_risk_group"]) * sub[
            "weight"
        ]
        return sub

    @staticmethod
    def check_bulk_risk_from_comp(comp: pd.DataFrame) -> pd.DataFrame:
    ...
    def check_not_monitored(self, ports, ppm, hs) -> pd.DataFrame:
        ...
        return self.check_not_monitored_from_comp(comp)

    def check_issuer_risk(self, ports, ppm, hs) -> pd.DataFrame:
        _, comp = ports.get_portfolio_health_score(ppm, hs, cal_comp=True)
        return self.check_issuer_risk_from_comp(comp)

    def check_bulk_risk(self, ports, ppm, hs) -> pd.DataFrame:
        _, comp = ports.get_portfolio_health_score(ppm, hs, cal_comp=True)
        return self.check_bulk_risk_from_comp(comp)
    ...
    def check_es_sell_list(self, ports, ppm, hs) -> pd.DataFrame:
        ...
        _, comp = ports.get_portfolio_health_score(ppm, hs, cal_comp=True)
        comp = comp.merge(
            self.es_sell_list, left_on="src_sharecodes", right_on="symbol", how="left"
        )
    ...
    def build_sell_recommendations(self, ports, ppm, hs) -> pd.DataFrame:
        ...
        bad_products = [
            self.check_not_monitored_from_comp(comp),
            self.check_issuer_risk_from_comp(comp),
            self.check_bulk_risk_from_comp(comp),
        ...
        to_sells = to_sell.groupby(
            ["port_id", "value", "weight"] + ports.prod_comp_keys,
            dropna=False,
            as_index=False,
        ).agg(
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            to_sells["amount"] = (
                -1.0
                * (
                    (to_sells["weight"] - to_sells["expected_weight"])
                    / to_sells["weight"]
                )
                * to_sells["value"]
            )
        ...
        to_sells = to_sells.join(
            ports.product_mapping, on="sec_id", how="left", rsuffix="_y"
        )
        to_sells.drop(columns="sec_id_y", inplace=True)
        ...
        for _, to_sell in to_sells.iterrows():
            ...
            ft = (
                to_sell["flag_tax_saving"].iloc[0]
                if "flag_tax_saving" in to_sell.columns
                else None
            )
            ...
            ptype_restricted = (not pd.isna(ptype)) and (
                ptype in ["Private Market", "Hedge Fund", "Structured Note"]
            )
            src_code = to_sell["src_sharecodes"].iloc[0]
            src_whitelisted = (not pd.isna(src_code)) and (
                src_code.lower() in self.product_whitelist
                if self.product_whitelist
                else False
            )
            ...
            funding = self.build_cash_proxy_funding(
                ports,
                to_sell[
                    [
                        c
                        for c in self.reco_cols
                        if c not in ["transaction_no", "batch_no"]
                    ]
                ],
                per_row=True,
            )
            ...
            recommendations.append(
                pd.concat([to_sell[cols], funding[cols]], ignore_index=True)
            )
    ...
    def build_buy_recommendations(self, ports, ppm) -> pd.DataFrame:
        ...
        if self.product_blacklist:
            prod_reco_rank = prod_reco_rank[
                ~prod_reco_rank["src_sharecodes"]
                .str.lower()
                .isin(self.product_blacklist)
            ]
        ...
        total_cash_w = (
            float(cash_buckets["weight"].sum()) if not cash_buckets.empty else 0.0
        )
        ...
        if (
            mandate_headroom > self.eps
            and total_cash_w > self.eps
            and not cash_buckets.empty
        ):
            for i in range(len(cash_buckets)):
                ...
                if not (
                    add_w >= self.min_weight_per_row
                    and abs(amount) >= self.min_amount_per_row
                ):
                    pass  # skip
                else:
                    chosen_md = self.select_product_discretionary_by_style(
                        ports=ports, currency=fund_ccy, headroom_amount=amount
                    )

                    if chosen_md is not None and add_w > self.eps:
                        sym = chosen_md["symbol"]
                        existing = ports.df_out[ports.df_out["symbol"] == sym][
                            ["weight"]
                        ]
                        cur_weight = (
                            float(existing["weight"].sum())
                        ...
                        cash_proxy = self.build_cash_proxy_funding(
                            ports, chosen, per_row=True
                        )
                        ...
                        trade = pd.concat(
                            [chosen[cols], cash_proxy[cols]], ignore_index=True
                        )
                        buys.append(trade)
        ...
        if cash_buckets is None or cash_buckets.empty:
            return (
                pd.concat(buys, ignore_index=True)
                if buys
                else pd.DataFrame(columns=self.reco_cols)
            )
        ...
        for _ in range(self.max_rebalance_loops):
            ...
            min_under_col = diffs[under_cols].idxmin(axis=1).iloc[0]
            min_under_val = float(
                diffs[min_under_col].iloc[0]
            )  # negative if underweight
            ...
            for _, cb in cash_buckets.iterrows():
                ...
                if (step_w < self.min_weight_per_row) or (
                    abs(step_amount) < self.min_amount_per_row
                ):
                    continue
                if step_w <= self.eps:
                    continue
                ...
                df_out = ports.df_out[
                    ["port_id", "src_sharecodes", "currency", "value", "weight"]
                ]
                ...
                if self.client_classification == "UI":
                    cand = cand[
                        ~(
                            (cand["is_ui"] == "N")
                            & (cand["asset_class_name"] == "Alternative")
                        )
                    ]
                else:
                    cand = cand[
                ...
                candidates = cand.merge(
                    df_out, on="src_sharecodes", how="left", suffixes=("", "_port")
                ...
                candidates = candidates[
                    candidates["expected_weight"] < self.single_line_cap_non_mandate
                ]
                if candidates.empty:
                    continue
                ...
                if (
                    "currency" not in chosen.columns
                    and "currency_port" in chosen.columns
                ):
                ...
                if len(chosen) > 1:
                    chosen = chosen.sample(
                        n=1, random_state=np.random.randint(0, 1_000_000)
                    )
                chosen = chosen[
                    [
                        c
                ...
                prod_reco_rank = prod_reco_rank[
                    ~prod_reco_rank["src_sharecodes"].isin(chosen["src_sharecodes"])
                ]
    ...
    def move_cash_overweight_to_proxy(self, ports, ppm) -> pd.DataFrame:
        cash_overweight = (
            (ports.df_out["product_type_desc"] == "Cash")
            | (ports.df_out["asset_class_name"] == "Cash and Cash Equivalent")
        ...
        for _, r in cash_rows.iterrows():
            ...
            if (move_w < self.min_weight_per_row) or (
                abs(amt_sell) < self.min_amount_per_row
            ):
                continue
            ...
            funding_buy = funding_buy[
                [c for c in self.reco_cols if c not in ["transaction_no", "batch_no"]]
            ]
            ...
            pairs.append(
                pd.concat([sell_row[cols], funding_buy[cols]], ignore_index=True)
            )
    ...
    def convert_cash_proxy_currency(self, ports) -> pd.DataFrame:
        ...
        for _, row in cash_proxy_rows.iterrows():
                ...
                [
                    {
                        ...
                        "sec_id": usd_proxy["sec_id"],
                        "src_sharecodes": usd_proxy["src_sharecodes"],
                        "desk": usd_proxy["desk"],
                        "port_type": usd_proxy["port_type"],
                        "currency": "USD",
                        "product_display_name": usd_proxy.get(
                            "product_display_name", np.nan
                        ),
            ...
            recommendations.append(
                pd.concat([sell_row[cols], buy_row[cols]], ignore_index=True)
            )
    ...
    def readable_flag(flag: str) -> str:
        ...
        # --- Default Fallback ---
        return (
            f"Rebalance portfolio according to internal investment guidelines ({flag})."
        )
    ...
    def rebalance(
        ...
        try:
            ...
            health_score_before = self.new_ports.get_portfolio_health_score(ppm, hs)[0][
                "health_score"
            ].values[0]
            ...
            if not new_money.empty:
                self.recommendations = pd.concat(
                    [self.recommendations, new_money], ignore_index=True
                )
                self.update_portfolio(self.new_ports, new_money)
            logger.debug(
                "[rebalance] new_money rows=%s",
                0 if new_money is None else len(new_money),
            )

            sells = self.build_sell_recommendations(self.new_ports, ppm, hs)
            if not sells.empty:
                self.recommendations = pd.concat(
                    [self.recommendations, sells], ignore_index=True
                )
            logger.debug(
                "[rebalance] sells rows=%s", 0 if sells is None else len(sells)
            )

            cash_shift = self.move_cash_overweight_to_proxy(self.new_ports, ppm)
            if not cash_shift.empty:
                self.recommendations = pd.concat(
                    [self.recommendations, cash_shift], ignore_index=True
                )
            logger.debug(
                "[rebalance] cash_shift rows=%s",
                0 if cash_shift is None else len(cash_shift),
            )

            convert_ccy = self.convert_cash_proxy_currency(self.new_ports)
            if not convert_ccy.empty:
                self.recommendations = pd.concat(
                    [self.recommendations, convert_ccy], ignore_index=True
                )
            logger.debug(
                "[rebalance] convert_ccy rows=%s",
                0 if convert_ccy is None else len(convert_ccy),
            )

            buys = self.build_buy_recommendations(self.new_ports, ppm)
            if not buys.empty:
                self.recommendations = pd.concat(
                    [self.recommendations, buys], ignore_index=True
                )
            logger.debug("[rebalance] buys rows=%s", 0 if buys is None else len(buys))

            logger.debug(
                "[rebalance] final recommendations rows=%s", len(self.recommendations)
            )

            # Merge product mapping for final output
            self.recommendations = self.recommendations.join(
                self.new_ports.product_mapping, on="sec_id", how="left", lsuffix="_reco"
            )
            self.recommendations = self.recommendations[self.reco_cols]

            health_score_after = self.new_ports.get_portfolio_health_score(ppm, hs)[0][
                "health_score"
            ].values[0]
            ...
            self.recommendations["flag_msg"] = self.recommendations["flag"].apply(
                type(self).readable_flag
            )

            try:
                logger.debug(
                    "[rebalance] final df_out rows=%s", len(self.new_ports.df_out)
                )
            except Exception:
