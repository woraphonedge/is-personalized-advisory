...
class PortfolioService:
    ...
    def get_port_id_from_customer_id(self, customer_id: int) -> int:
        result = self.portfolios.port_id_mapping[
            self.portfolios.port_id_mapping["customer_id"] == customer_id
        ]["port_id"].values
        ...
        if len(result) > 1:
            raise ValueError(
                f"More than one portfolio found for customer_id={customer_id}"
            )
        return int(result[0])
    ...
    def get_client_portfolio(self, customer_id: int) -> Portfolios:
        ...
        # slice the data
        df_out_client = self.portfolios.df_out[
            self.portfolios.df_out["port_id"] == port_id
        ]
        df_style_client = self.portfolios.df_style[
            self.portfolios.df_style["port_id"] == port_id
        ]
        port_id_mapping = self.portfolios.port_id_mapping[
            self.portfolios.port_id_mapping["port_id"] == port_id
        ]
        ...
        client_portfolio.set_portfolio(
            df_out_client, df_style_client, port_ids, port_id_mapping
        )
